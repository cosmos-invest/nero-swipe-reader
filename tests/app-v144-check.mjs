...
assert.match(commentsUi,/rows\.slice\(0,S\.priorityVisible\)\.forEach\(\(row,index\)=>/);
assert.match(commentsUi,/A\.startReader&&A\.startReader\(rows\.slice\(\),index\)/,'each comment-partner card must open the full filtered queue at its index');
assert.doesNotMatch(commentsUi,/A\.startReader&&A\.startReader\(\[row\],0\)/,'single-row Reader queue causes Next to reopen the same article');
