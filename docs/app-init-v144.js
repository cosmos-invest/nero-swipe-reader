(() => {
  ...
  $('priorityRefresh').addEventListener('click',()=>A.scanPriority(false));$('priorityCancel').addEventListener('click',()=>A.rpc('cancel_job',{kind:'priority'}).catch(()=>{}));$('priorityStart').addEventListener('click',()=>A.startPriorityReader());$('priorityMore').addEventListener('click',()=>{S.priorityVisible+=A.PAGE_SIZE;A.renderPriority();});
